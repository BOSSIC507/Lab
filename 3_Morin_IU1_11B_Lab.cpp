...
struct Product{ //создаем структуру для блюд, компоненты -- название, тип и цена 
    string name;
    string type1;
    double price;
...
void addProductToFile(const Product& product) { //функция добавления блюда в файл
    ofstream outFile("products.txt", ios::app); //ios::app -- запись происходит в конец файла
    if (outFile.is_open()) {
        outFile << product.name << "," << product.type1 << "," << product.price << endl;
        outFile.close();
        cout << "Блюдо успешно добавлен." << endl;
    } else {
...
void searchProductByPrice(const double& maxPrice) { //функция поиска блюда по цене
    ...
    if (inFile.is_open()) {
        ...
        inFile.close();
        if (!found) {
            cout << "Блюда, цена которых меньше заданной, не найдены." << endl;
        }
    } else {
...
void sortProductbyNumber() {
    ...
    if (inFile.is_open()) {
        ...
        string lines[100]; // Предполагаем, что не более 100 блюд
        int count = 0;
        string line;
        ...
        if (outFile.is_open()) {
            ...
            outFile.close();
            cout << "Блюда отсортированы по количеству." << endl;
        } else {
            cerr << "Не удалось открыть файл для записи." << endl;
        }
    } else {
    ...
    do {
        ...
        switch (choice) {
            case 1: {
                ...
                Product newProduct = {name, type1, price};
